...
#include <RangeQuery1.h>
#include <TopkQuery1.h>


using namespace std;

class Interrogator_V1 {
	
	int8_t versionID;
	vector<vector<pair<string, string>>> compactStructure;
	vector<RangeQuery_V1> range_set;
	vector<TopkQuery_V1> topk_set;
	string refDate;
	int32_t normalize;
	int32_t refYear = 0;
	int32_t refMonth = 0;
	                    ...
	                    const string & K)
	{
		...
		if (operation == "range")
		{
			RangeQuery_V1 query(compressDate(time1) + normalize, compressDate(time2) + normalize, page);
		...
		{	
			int64_t k = stoi(K);
			if (k <= 0) return;
			TopkQuery_V1 query(compressDate(time1) + normalize, compressDate(time2) + normalize, page, k);
...
public:
	...
	Interrogator_V1 (int8_t versionID) {
