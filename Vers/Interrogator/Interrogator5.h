...
#include <RangeQuery5.h>
#include <TopkQuery5.h>
#include <cmph.h>
#include <algorithm>
...
inline bool comp_V5(const vector <char> & c1, const vector<char> & c2)
{
    return strcmp(c1.data(), c2.data()) < 0;
}

class Interrogator_V5 {
	...
	vector<RangeQuery_V5> range_set;
	vector<TopkQuery_V5> topk_set;
	cmph_t *hash;
	                    ...
	                    const string & K)
	{
		...
		if (operation == "range")
		{
			RangeQuery_V5 query(time1,time2, page, hash, keys_Chrono_ID, keys_ID_Chrono);
		...
		{	
			int64_t k = stoi(K);
			if (k <= 0) return;
			TopkQuery_V5 query(time1,time2, page, k, hash, keys_Chrono_ID, keys_ID_Chrono);
...
public:
	...
	Interrogator_V5 (int8_t versionID) {
		
		this->versionID = versionID;
		ofstream out("result_005.txt");
	}
	...
	inline void deserialize() {
		...
		listPerDate = ref(tmp);
		sort(cKeys.begin(), cKeys.end(), comp_V5);
	...
	inline void performQueries() 
	{
		...
		for (RangeQuery_V5 query : range_set)
		{
			query.performQuery(listPerDate, delimPerDate);
		}

		for (TopkQuery_V5 query : topk_set)
		{
			query.performQuery(listPerDate, delimPerDate);
		}
