...
#include <RangeQuery6.h>
#include <TopkQuery6.h>
#include <cmph.h>
#include <algorithm>
...
class Interrogator_V6 {
	
	int8_t versionID;
	vector<RangeQuery_V6> range_set;
	vector<TopkQuery_V6> topk_set;
	cmph_t *hash;
	                    ...
	                    const string & K)
	{
		...
		if (operation == "range")
		{
			RangeQuery_V6 query(dateCheck[time1],dateCheck[time2], cmph_search(hash, page.c_str(), (cmph_uint32)strlen(page.c_str())), page);
		...
		{	
			int64_t k = stoi(K);
			if (k <= 0) return;
			TopkQuery_V6 query(dateCheck[time1],dateCheck[time2], cmph_search(hash, page.c_str(), (cmph_uint32)strlen(page.c_str())), k, page);
...
public:
	...
	Interrogator_V6 (int8_t versionID) {
		
		this->versionID = versionID;
		ofstream out("result_006.txt");
	}
	...
	{	
		...
		for (RangeQuery_V6 query : range_set)
		{
			chrono::time_point<chrono::system_clock> start, end; 
			start = chrono::system_clock::now(); 
			
				query.performQuery(Structure);
		...
		for (TopkQuery_V6 query : topk_set)
		{
			chrono::time_point<chrono::system_clock> start, end; 
			start = chrono::system_clock::now(); 
	...
	~Interrogator_V6 () {
		vector<RangeQuery_V6>().swap(range_set);
		vector<TopkQuery_V6>().swap(topk_set);
	    cmph_destroy(hash);	
